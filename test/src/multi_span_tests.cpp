...
TEST_CASE("from_nullptr_length_constructor")
{
...

  {
    ...
    CHECK_THROWS_AS(workaround_macro(), std::logic_error);

    auto const_workaround_macro = []() {
      const multi_span<const int> cs{nullptr, 1};
    };
    CHECK_THROWS_AS(const_workaround_macro(), std::logic_error);
  }

  {
    ...
    auto const_workaround_macro = []() {
      const multi_span<const int, 0> s{nullptr, 1};
    };
    CHECK_THROWS_AS(const_workaround_macro(), std::logic_error);
  }
...
TEST_CASE("from_pointer_pointer_constructor")
{
...

  {
    auto workaround_macro = [&]() {
      const multi_span<int> s{&arr[1], &arr[0]};
    };
    CHECK_THROWS_AS(workaround_macro(), std::logic_error);
  }
...
TEST_CASE("from_dynamic_array_constructor")
{
...

  {
    ...
    CHECK_THROWS_AS(s[10][3][4], std::logic_error);
  }
...
TEST_CASE("from_std_array_constructor")
{
  ...
  // we revise the multidimensional interface and what transformations between
  // dimensionality look like
  //{
  //    multi_span<int, 2, 2> s{arr};
  //    CHECK(s.size() == narrow_cast<ptrdiff_t>(arr.size()) && s.data() ==
  //    arr.data());
  //}
...
TEST_CASE("from_convertible_span_constructor")
{
  ...
  CHECK_THROWS_AS(f(), std::logic_error);
#endif
...
#endif
    CHECK_THROWS_AS(av.first(6).length(), std::logic_error);
  }
...
#endif
    CHECK_THROWS_AS(av.last(6).length(), std::logic_error);
  }
...
TEST_CASE("subspan")
{
...

  {
    ...
    CHECK(av.subspan(0, 5).length() == 5);
    CHECK_THROWS_AS(av.subspan(0, 6).length(), std::logic_error);
    CHECK_THROWS_AS(av.subspan(1, 5).length(), std::logic_error);
  }

  {
    ...
    CHECK(av.subspan(5, 0).length() == 0);
    CHECK_THROWS_AS(av.subspan(6, 0).length(), std::logic_error);
  }

  {
    ...
    CHECK((av.subspan<0, 0>().length() == 0));
    CHECK(av.subspan(0, 0).length() == 0);
    CHECK_THROWS_AS((av.subspan<1, 0>().length()), std::logic_error);
  }

  {
    multi_span<int> av;
    CHECK(av.subspan(0).length() == 0);
    CHECK_THROWS_AS(av.subspan(1).length(), std::logic_error);
  }

  {
    ...
    CHECK_THROWS_AS(av.subspan(6).length(), std::logic_error);
...
{
  {
    ...
    CHECK(s.extent() == 0);
    CHECK(s.extent(0) == 0);
    CHECK_THROWS_AS(s.extent(1), std::logic_error);
#ifdef CONFIRM_COMPILATION_ERRORS
    CHECK(s.extent<1>() == 0);
#endif
  }
...

  {
    ...
    CHECK(s.extent(0) == 1);
    CHECK(s.extent(1) == 2);
    CHECK_THROWS_AS(s.extent(3), std::logic_error);
  }
...
TEST_CASE("operator_function_call")
{
...

  {
    multi_span<int> s = arr;
    CHECK(s(0) == 1);
    CHECK_THROWS_AS(s(5), std::logic_error);
  }
...
TEST_CASE("bounds_checks")
{
  ...
  // out of bounds
  CHECK_THROWS_AS(av[1][3] = 3, std::logic_error);
  CHECK_THROWS_AS((av[{1, 3}] = 3), std::logic_error);

  CHECK_THROWS_AS(av[10][2], std::logic_error);
  CHECK_THROWS_AS((av[{10, 2}]), std::logic_error);

  CHECK_THROWS_AS(av[-1][0], std::logic_error);
  CHECK_THROWS_AS((av[{-1, 0}]), std::logic_error);
...
TEST_CASE("md_access")
{
  ...
  auto image_view = as_multi_span(
    as_multi_span(image_ptr, imgSize), dim(height), dim(width), dim<3>());
...
{
  {
    ...
    CHECK_THROWS_AS(empty_av[0], std::logic_error);
    CHECK_THROWS_AS(empty_av.begin()[0], std::logic_error);
    CHECK_THROWS_AS(empty_av.cbegin()[0], std::logic_error);
    for (auto& v : empty_av)
    {
      (void)v;
      CHECK(false);
...
TEST_CASE("index_operations")
{
  ...
  {
    multi_span_index<2> k = detail::shift_left(i);
...
TEST_CASE("dynamic_span_section_iteration")
{
  ...
  // first bound is dynamic
  {
    multi_span<int, dynamic_range, 2> av2 =
      as_multi_span(av, dim(height), dim(width));
    iterate_second_column(av2);
  }
  // second bound is dynamic
  {
    multi_span<int, 4, dynamic_range> av2 =
      as_multi_span(av, dim(height), dim(width));
    iterate_second_column(av2);
  }
  // both bounds are dynamic
...
TEST_CASE("span_structure_size")
{
  ...
  CHECK_THROWS_AS(av1[10][3][4], std::logic_error);
...
// initialization or assignment to static multi_span that REDUCES size is NOT ok
  ...
  // ...or dynamically
  {
    // NB: implicit conversion to multi_span<int,2> from
    // multi_span<int,dynamic_range>
    multi_span<int, 1> av2 = av4.first(1);
    (void)av2;
  }

  // initialization or assignment to static multi_span that requires size
  // INCREASE is not ok.
  int arr2[2] = {1, 2};
