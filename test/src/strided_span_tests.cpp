...
TEST_CASE("span_section_test")
{
    ...
    const auto sub = av.section({15, 0, 0}, multi_span_index<3>{2, 2, 2});
    const auto subsub = sub.section({1, 0, 0}, multi_span_index<3>{1, 1, 1});
    (void) subsub;
}
...
TEST_CASE("strided_span_bounds")
{
...

    {
        ...
        CHECK_THROWS_AS(av.section(0, 0)[0], std::logic_error);
        CHECK_THROWS_AS(av.section(1, 0)[0], std::logic_error);
        CHECK_THROWS_AS(av.section(1, 1)[1], std::logic_error);

        CHECK_THROWS_AS(av.section(2, 5), std::logic_error);
        CHECK_THROWS_AS(av.section(5, 2), std::logic_error);
        CHECK_THROWS_AS(av.section(5, 0), std::logic_error);
        CHECK_THROWS_AS(av.section(0, 5), std::logic_error);
        CHECK_THROWS_AS(av.section(5, 5), std::logic_error);
    }

    {
        ...
        CHECK(sav[0] == 0);
        CHECK(sav[3] == 0);
        CHECK_THROWS_AS(sav[4], std::logic_error);
    }

    {
        // zero extent
        strided_span<int, 1> sav{av, {{}, {1}}};
        CHECK_THROWS_AS(sav[0], std::logic_error);
    }
...

    {
        ...
        CHECK(sav.bounds().index_bounds() == multi_span_index<1>{2});
        CHECK(sav[1] == 1);
        CHECK_THROWS_AS(sav[2], std::logic_error);
    }
...

    {
        ...
        CHECK_THROWS_AS((strided_span<int, 1>{arr, {3, 2}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{arr, {3, 3}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{arr, {4, 5}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{arr, {5, 1}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{arr, {5, 5}}), std::logic_error);
    }

    {
        // bounds cross data boundaries - from array view
        CHECK_THROWS_AS((strided_span<int, 1>{av, {3, 2}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av, {3, 3}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av, {4, 5}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av, {5, 1}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av, {5, 5}}), std::logic_error);
    }

    {
        // bounds cross data boundaries - from dynamic arrays
        CHECK_THROWS_AS((strided_span<int, 1>{av.data(), 4, {3, 2}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av.data(), 4, {3, 3}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av.data(), 4, {4, 5}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av.data(), 4, {5, 1}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av.data(), 4, {5, 5}}), std::logic_error);
        CHECK_THROWS_AS((strided_span<int, 1>{av.data(), 2, {2, 2}}), std::logic_error);
    }
...
{
    {
        ...
        CHECK_THROWS_AS(empty_sav[0], std::logic_error);
        CHECK_THROWS_AS(empty_sav.begin()[0], std::logic_error);
        CHECK_THROWS_AS(empty_sav.cbegin()[0], std::logic_error);
